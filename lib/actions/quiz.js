...
export async function createQuiz(formData) {
  try {
    ...
    console.log("Created Quiz Document:", newQuiz);
...
export async function deleteQuiz(formData) {
  try {
    ...
    await connectMongoDB();
    const updated = await Quiz.findByIdAndUpdate(id, { deletedAt: new Date() }, { new: true });
    console.log("Soft delete result:", updated ? "Success" : "Failed (Not Found)");
    revalidatePath("/admin/quiz");
    revalidatePath("/quiz");
